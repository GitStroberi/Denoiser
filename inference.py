...
# Import your split model definitions and helper functions.
# Ensure that inference_split_model_definition.py is in the Python path.
from model_def import (
    SAMPLE_RATE, TARGET_AUDIO_LENGTH, FRAME_LENGTH,
    compute_valid_length,
    get_upsample_kernel, get_downsample_kernel, upsample2, downsample2,
    CausalDemucsSplit,
    DemucsEncoderONNX,
    DemucsDecoderONNX
)
...
SAMPLE_RATE = 16000
# FRAME_LENGTH was computed inside your model definition file:
# For 2 seconds, FRAME_LENGTH is the valid input length computed from TARGET_AUDIO_LENGTH.
print("Frame length (samples):", FRAME_LENGTH)

##############################
# External Normalization Function (NumPy version)
##############################
...
# Create an instance of your split model and load weights.
...
# Load ONNX Runtime sessions for the encoder and decoder.
# Assume these files have been exported beforehand.
#encoder_onnx_path = "demucs_encoder.onnx"
#decoder_onnx_path = "demucs_decoder.onnx"
#assert os.path.exists(encoder_onnx_path), f"Encoder ONNX file {encoder_onnx_path} not found!"
#assert os.path.exists(decoder_onnx_path), f"Decoder ONNX file {decoder_onnx_path} not found!"
...
# Our encoder returns (latent, skip1, skip2, ..., skip_{depth}) => total outputs = depth+1.
...
with torch.no_grad():
    ...
    latent_dummy = dummy_encoder_outputs[0]  # Already a tensor of shape (1, C, T_enc)
    latent_tensor = latent_dummy.float().to(device)
    latent_seq = latent_tensor.permute(0, 2, 1)  # Shape: (B, T_enc, C)
    lstm_out, _ = model.lstm(latent_seq)
    lstm_out = lstm_out.permute(0, 2, 1)  # Back to (B, C, T_enc)
    # Combine LSTM output with original latent (element-wise addition)
...
##############################
# Global LSTM State (for stateful inference)
##############################
lstm_state = None
...
def audio_callback(indata, outdata, frames, time_info, status):
    ...
    if status:
        print("Status:", status)
    # indata has shape (frames, channels); ensure shape (1,1,frames)
    noisy_audio = np.expand_dims(indata.T, axis=0).astype(np.float32)  # shape: (1, 1, frames)
    
    # External normalization on the incoming audio.
    ...
    # Reshape to (frames, 1) for audio output.
    ...
    # Optional: print processing time for debugging.
    # (Uncomment if needed)
    # print(f"Processed a block in {time.time() - start_time:.4f} sec")
