...
# Demucs design parameters (for our causal model)
...
class CausalDemucsSplit(nn.Module):
    def __init__(self, chin=1, chout=1, hidden=H, depth=L, kernel_size=K, stride=S_conv,
        ...
        """
        Causal Demucs model split into encoder, GRU, and decoder.
        The encoder and decoder parts are ONNX-compatible (fixed input size).
        The GRU bottleneck remains in PyTorch and can be executed on the CPU.
        """
        super().__init__()
        ...
        for i in range(depth):
            ...
            ch *= growth
        # GRU bottleneck with input size equal to the output of the final encoder layer.
        self.gru = nn.GRU(input_size=encoder_channels[-1], hidden_size=encoder_channels[-1],
    ...
    def encode(self, audio):
        ...
        Returns:
           latent: output of the final encoder layer (to be fed into the GRU)
    ...
    def process_bottleneck(self, latent):
        """
        Process the encoder latent representation through the GRU.
        """
        B, feat, seq = latent.shape
        x = latent.permute(0, 2, 1)  # shape: (B, seq, feat)
        x, _ = self.gru(x)
        x = x.permute(0, 2, 1)      # back to (B, feat, seq)
        return x

    def decode(self, x, skip_list, orig_len):
        """
        Decode the GRU-processed latent using the stored skip connections.
        """
    ...
    def forward(self, audio):
        """
        Full end-to-end forward pass: encode -> GRU bottleneck -> decode.
        """
...
##########################
# These wrappers allow you to export the encoder and decoder separately.
...
class DemucsDecoderONNX(nn.Module):
    ...
    def forward(self, *inputs):
        # Expect inputs: first is the GRU output (to be added with latent) then the skip connections.
        # We use FRAME_LENGTH as the constant original length.
        latent_out = inputs[0]
