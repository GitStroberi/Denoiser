...
# New imports for TensorBoard logging and image conversion.
...
class ValentiniDataset(torch.utils.data.Dataset):
    ...
    def _synchronized_crop_or_pad(self, noisy, clean):
        # Assume both have the same length (if not, we could take the minimum)
...
# Set dataset paths (adjust these paths to your local environment)
DATA_ROOT = "/home/afilip/datasets/valentini"
...
try:
    torch.onnx.export(
        export_model, dummy_input, "denoiser_causal_split.onnx",
        opset_version=9,
...
# -----------------------------------------------------------------------------------
# 6. TRAINING & VALIDATION LOOP (modified to include grouped TensorBoard logging)
# -----------------------------------------------------------------------------------
...
best_model_path = "best_model_split.pth"
